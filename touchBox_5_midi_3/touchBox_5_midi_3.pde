...
float getPoleValue()
{
  ...
  {
// delay(100);
  /*  float midiValue = (tmpValue+tmpValueH)/2.0f;
    if(false && abs(oldvalue - midiValue) > 0.05)
    {
      //lastnote = millis() + 0;
      float note = map(midiValue, 0.0f,1.0f,10,60);
      //float span_old = map(oldvalue, 0.0f,1.0f,30,60);
       noteOn(0x90, oldNote, 0); 
      noteOn(0x90, note, 125.0f);
      oldNote = note;
    }*/
  
  
 //   oldvalue = midiValue;
    setControlValue(4,round(tmpValue*127.0f));
    setControlValue(5,round(tmpValueH*127.0f));
  }
